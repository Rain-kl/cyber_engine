...
@command("/approve")
async def approve(
    chat_completion_request: ChatCompletionRequest, websocket: WebSocket, chunk_wrapper
):
    """批准用户最新任务"""
    try:
        mcp_tool_call = MCPToolCall()
        try:
            task = task_center.get_task(
                chat_completion_request.extra_headers.authorization
            )
        ...
        for tool_call in tool_chain:
            ...
            try:
                ...
                if tool_result:
                    content = (
                        f"已执行函数{tool_name}, 参数: {tool_args}, 结果: {tool_result}"
                    )
                else:
...
async def get_all_users_history(
    ...
    try:
        ...
        # 发送所有用户的历史记录
        await manager.send_private_stream(
            chunk_wrapper.content_chunk_wrapper("所有用户的历史记录:\n"), websocket
        )
