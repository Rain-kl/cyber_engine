...
class RedisSqlite(RedisABC):
            ...
            if key:
                ...
                if result:
                    ...
                    cursor = await db.execute(
                        "SELECT COUNT(*) FROM list WHERE key = ?", (key,)
                    )
                    result = await cursor.fetchone()
    ...
    async def get(self, key: str) -> str | list | None:
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                f"SELECT value FROM kv WHERE key = ?", (key,)
            ) as cursor:
    ...
    async def lpush(self, key, value):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                "SELECT MIN(position) FROM list WHERE key = ?", (key,)
            ) as cursor:
                row = await cursor.fetchone()
                position = row[0] - 1 if row[0] is not None else 0
            await db.execute(
                "INSERT INTO list (key, value, position) VALUES (?, ?, ?)",
                (key, value, position),
            )
            await db.commit()
    ...
    async def rpush(self, key, value):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                "SELECT MAX(position) FROM list WHERE key = ?", (key,)
            ) as cursor:
                row = await cursor.fetchone()
                position = row[0] + 1 if row[0] is not None else 0
            await db.execute(
    ...
    async def get_list(self, key) -> list:
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT value FROM list WHERE key = ?", (key,)
            ) as cursor:
    ...
    async def rpop(self, key):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT value, MAX(position) FROM list WHERE key = ?", (key,)
            ) as cursor:
                row = await cursor.fetchone()
                if row and row[0] is not None:
                    value = row[0]
                    await db.execute(
                        "DELETE FROM list WHERE key = ? AND position = ?", (key, row[1])
                    )
                    await db.commit()
                    return value
                return None
    ...
    async def lpop(self, key):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                "SELECT value, MIN(position) FROM list WHERE key = ?", (key,)
            ) as cursor:
                row = await cursor.fetchone()
                if row and row[0] is not None:
                    value = row[0]
