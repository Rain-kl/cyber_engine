            ...
            async def async_wrapper(*args, **kwargs) -> Any:
                for count in range(1, retries + 1):
                    try:
                        ...
                        logger.error(f"{_log} __{func.__name__}__ Time out {count}: {str(e)}")
...
class AsyncSpider:
    ...
    async def get(self, path: str, params=None) -> httpx.Response:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            response = await client.get(self.base_url + path, headers=self.headers, timeout=self.timeout, params=params)
            return response

    async def post(self, path: str, data=None, _json=None) -> httpx.Response:
        assert path.startswith('/'), "path should not start with /"
        async with httpx.AsyncClient(verify=False) as client:
            response: httpx.Response = await client.post(self.base_url + path, headers=self.headers,
